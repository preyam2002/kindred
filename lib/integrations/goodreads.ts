...
import type { GoodreadsBook, GoodreadsProfile } from "@/lib/scrapers/goodreads-scraper";
...
/**
 * Import Goodreads scraped data and sync to database
 */
export async function importGoodreadsScraped(
  userId: string,
  ...
  try {
    ...
    if (profile.books.length === 0) {
      throw new Error("No books found in scraped profile");
    }

    // Get or create source record
    const profileUrl = goodreadsProfileUrl || `https://www.goodreads.com/user/show/${profile.userId}`;
    const { data: existingSource } = await supabase
      .from("sources")
      .select("id")
      .eq("user_id", userId)
    ...
    // Step 1: Batch check which books already exist
    const bookIds = profile.books.map(b => b.bookId).filter(Boolean);
    const { data: existingBooks } = await supabase
      .from("books")
      .select("id, source_item_id, poster_url")
      .eq("source", "goodreads")
    ...
    // Step 3: Prepare data for all books
    const booksNeedingCovers: Array<{ book: GoodreadsBook; mediaId: string }> = [];
    ...
    for (const book of profile.books) {
      ...
      if (existing) {
        bookMap.set(book.bookId, { id: existing.id, needsCover: !existing.poster_url });
        if (!existing.poster_url && !book.coverUrl) {
          booksNeedingCovers.push({ book, mediaId: existing.id });
        }
      } else {
        newBooks.push({
    ...
    for (let i = 0; i < booksNeedingCovers.length; i += BATCH_SIZE) {
      ...
      await Promise.all(
        batch.map(async ({ book, mediaId }) => {
          try {
            const coverResult = await fetchBookCover(book.title, book.author);
            if (coverResult.poster_url) {
              coverResults.set(mediaId, coverResult.poster_url);
            }
          } catch (error) {
    ...
    for (let i = 0; i < newBooks.length; i += BATCH_SIZE) {
      ...
      await Promise.all(
        batch.map(async (item) => {
          const book = profile.books.find(b => b.bookId === item.source_item_id);
          if (book && !item.poster_url) {
            try {
              const coverResult = await fetchBookCover(book.title, book.author);
              if (coverResult.poster_url) {
                newCovers.set(item.source_item_id, coverResult.poster_url);
    ...
    console.error("Error importing Goodreads scraped data:", error);
    throw error;
  }
}
