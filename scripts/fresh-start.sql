...
-- 
-- This script:
-- 1. Drops ALL existing database objects (tables, functions, triggers, policies, etc.)
-- 2. Recreates everything from scratch with correct schemas
-- 
...
--   3. Click "Run"
--
-- This ensures a completely clean database with all correct schemas.

-- ============================================================================
-- PART 1: DROP EVERYTHING (Complete Cleanup)
-- ============================================================================
...
-- Drop ALL tables dynamically (to catch any we might have missed)
DO $$
DECLARE
    r RECORD;
BEGIN
...
-- Also explicitly drop known tables (in case dynamic drop missed some)
...
-- Drop all custom types (after tables are dropped)
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (
        ...
        AND t.typtype = 'c'
        AND NOT EXISTS (
            -- Skip types that are still referenced by tables
        ...
        EXCEPTION WHEN OTHERS THEN
            -- Continue if type is still referenced
            NULL;
        END;
    END LOOP;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE 'All existing database objects dropped. Starting fresh creation...';
END $$;

-- ============================================================================
-- PART 2: CREATE BASE SCHEMA
-- ============================================================================
...
-- ============================================================================
-- PART 7: APPLY MIGRATIONS
-- ============================================================================

-- Add user_media tracking fields
...
-- Collections, Queue, Friends, Activity
...
BEGIN
    ...
    RAISE NOTICE '✅ Fresh start complete! Created % tables.', table_count;
    RAISE NOTICE '✅ All schemas are correct and up-to-date.';
END $$;
